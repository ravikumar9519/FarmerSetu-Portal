...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      console.log("Token exists, navigating to home page");
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg'>
        ...
        <button className='w-full py-2 my-2 text-base text-white rounded-md bg-primary'>{state === 'Sign Up' ? 'Create account' : 'Login'}</button>
        {state === 'Sign Up'
